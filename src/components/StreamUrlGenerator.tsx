...
import { StreamConfig, ProtocolType, EncryptionType, HistoryRecord, AllProtocolUrls } from '../types';
import { generateStreamUrl, generateAllProtocolUrls, validateStreamConfig } from '../utils/urlGenerator';
import { saveLastConfig, getLastConfig, saveAggregatedHistoryRecord } from '../utils/storage';
...
const StreamUrlGenerator: React.FC<StreamUrlGeneratorProps> = ({ onHistoryClick }) => {
  ...
  const protocolOptions: { value: ProtocolType; label: string }[] = [
    ...
    { value: 'SRT', label: 'SRT' },
    { value: 'RTMP_OVER_SRT', label: 'RTMP over SRT' },
    { value: 'RTMP_OVER_QUIC', label: 'RTMP over QUIC' }
  ];
  ...
  useEffect(() => {
    ...
    if (lastConfig) {
      form.setFieldsValue({
        ...lastConfig,
        expireTime: lastConfig.expireTime ? dayjs(lastConfig.expireTime + ' UTC') : null
      });
    } else {
      // 设置默认值
      form.setFieldsValue({
        ...
        >
          ...
          <Row gutter={16}>
            <Col xs={24} sm={8}>
              ...
              >
                <Input placeholder="如: push.example.com" />
              </Form.Item>
            </Col>
